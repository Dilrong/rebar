...
export function LibrarySelectionToolbar({
  ...
  return (
    <section className="mb-8 border-4 border-foreground bg-foreground p-4 text-background shadow-brutal-sm md:shadow-brutal">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2 border-b-4 border-background/30 pb-4">
        <div>
          <p className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-background/70">04 / BATCH OPS</p>
          <p className="mt-2 font-black uppercase text-xl">{selectedCount} {t("library.selected", "선택됨")}</p>
        </div>
        <div className="grid w-full grid-cols-1 gap-2 sm:w-auto sm:grid-cols-2">
          <button type="button" onClick={onSelectVisible} className="min-h-[44px] border-4 border-background bg-background px-3 py-2 font-mono text-xs font-bold uppercase text-foreground hover:bg-accent hover:text-accent-foreground shadow-[4px_4px_0_0_rgba(255,255,255,0.15)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 active:translate-y-1 active:translate-x-1 transition-all duration-200">
            {t("library.selectVisible", "보이는 항목 선택")}
          </button>
          <button type="button" onClick={onClearSelection} className="min-h-[44px] border-4 border-background bg-background px-3 py-2 font-mono text-xs font-bold uppercase text-foreground hover:bg-foreground hover:text-background shadow-[4px_4px_0_0_rgba(255,255,255,0.15)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 active:translate-y-1 active:translate-x-1 transition-all duration-200">
      ...
      <div className="mb-3 grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <button type="button" onClick={() => onApplyBulkState("TRASHED")} className="min-h-[44px] border-4 border-background bg-background px-3 py-2 font-mono text-xs font-bold uppercase text-destructive hover:border-destructive hover:bg-destructive hover:text-destructive-foreground shadow-[4px_4px_0_0_rgba(255,255,255,0.15)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 active:translate-y-1 active:translate-x-1 transition-all duration-200">
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_auto_auto]">
        ...
        <select
          ...
          className="min-h-[44px] border-4 border-background bg-background px-3 py-2 font-mono text-xs text-foreground focus:outline-none focus:ring-0 shadow-[inset_4px_4px_0px_0px_rgba(0,0,0,1)] focus:shadow-none focus:translate-x-1 focus:translate-y-1 transition-all duration-200 rounded-none cursor-pointer appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22currentColor%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E')] bg-[length:0.65em_0.65em] bg-[right_1rem_center] bg-no-repeat pr-10 sm:col-span-2 xl:col-span-1 dark:shadow-[inset_4px_4px_0px_0px_rgba(255,255,255,0.1)]"
        >
